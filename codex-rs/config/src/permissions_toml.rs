...
pub struct NetworkProxyConfig {
    pub network: NetworkProxyRuntimeConfig,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct NetworkProxyRuntimeConfig {
    pub enabled: bool,
    ...
    pub domains: Option<ProxyNetworkDomainPermissions>,
    pub unix_sockets: Option<ProxyNetworkUnixSocketPermissions>,
    pub allow_local_binding: bool,
    pub mitm_hooks: Vec<MitmHookConfig>,
    pub mitm: bool,
}

impl Default for NetworkProxyRuntimeConfig {
    fn default() -> Self {
        Self {
            enabled: true,
...
impl NetworkProxyRuntimeConfig {
    fn upsert_domain_permission(
        &mut self,
        pattern: String,
        permission: ProxyNetworkDomainPermission,
    ) {
        let domains = self.domains.get_or_insert_with(Default::default);
        domains.entries.insert(normalize_host(&pattern), permission);
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ProxyNetworkDomainPermission {
    Allow,
    Deny,
}

#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct ProxyNetworkDomainPermissions {
    pub entries: BTreeMap<String, ProxyNetworkDomainPermission>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ProxyNetworkUnixSocketPermission {
    Allow,
    None,
}

#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct ProxyNetworkUnixSocketPermissions {
    pub entries: BTreeMap<String, ProxyNetworkUnixSocketPermission>,
}
