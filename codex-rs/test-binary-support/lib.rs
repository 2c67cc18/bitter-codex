    ...
    match classify(exe_name, argv1.as_deref().and_then(|arg| arg.to_str())) {
        ...
        TestBinaryDispatchMode::InstallAliases => {
            let codex_home = match tempfile::Builder::new().prefix(codex_home_prefix).tempdir() {
                ...
                Err(error) => panic!("failed to create test BITTER_CODEX_HOME: {error}"),
            };
            let previous_codex_home = std::env::var_os("BITTER_CODEX_HOME");

            unsafe {
                std::env::set_var("BITTER_CODEX_HOME", codex_home.path());
            }
            ...
            match previous_codex_home.as_ref() {
                Some(value) => unsafe {
                    std::env::set_var("BITTER_CODEX_HOME", value);
                },
                None => unsafe {
                    std::env::remove_var("BITTER_CODEX_HOME");
                },
            }
