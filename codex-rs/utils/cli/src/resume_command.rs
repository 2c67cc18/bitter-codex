...
pub fn resume_command(thread_name: Option<&str>, thread_id: Option<ThreadId>) -> Option<String> {
    ...
    resume_target.map(|target| {
        ...
        if needs_double_dash {
            format!("bitter-codex resume -- {escaped}")
        } else {
            format!("bitter-codex resume {escaped}")
        }
    })
...
pub fn resume_hint(thread_name: Option<&str>, thread_id: Option<ThreadId>) -> Option<String> {
    ...
    match thread_name.filter(|name| !name.is_empty()) {
        Some(thread_name) => Some(format!(
            "bitter-codex resume, then select {thread_name} ({thread_id})"
        )),
...
mod tests {
    ...
    fn prefers_name_over_id() {
        ...
        assert_eq!(command, Some("bitter-codex resume my-thread".to_string()));
    }
    ...
    fn formats_thread_id_when_name_is_missing() {
        ...
        assert_eq!(
            command,
            Some("bitter-codex resume 123e4567-e89b-12d3-a456-426614174000".to_string())
        );
    }
    ...
    fn quotes_thread_names_when_needed() {
        ...
        assert_eq!(
            command,
            Some("bitter-codex resume -- -starts-with-dash".to_string())
        );

        let command = resume_command(Some("two words"), None);
        assert_eq!(command, Some("bitter-codex resume 'two words'".to_string()));

        let command = resume_command(Some("quote'case"), None);
        assert_eq!(
            command,
            Some("bitter-codex resume 'quote'\"'\"'case'".to_string())
        );

        let command = resume_command(Some("$(echo bad)`x`"), None);
        assert_eq!(
            command,
            Some("bitter-codex resume '$(echo bad)`x`'".to_string())
        );
    }
    ...
    fn resume_hint_names_picker_item_with_id() {
        ...
        assert_eq!(
            hint,
            Some(
                "bitter-codex resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)"
                    .to_string()
            )
        );
    }
