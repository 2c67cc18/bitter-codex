...
impl PermissionsInstructions {
    pub(crate) fn from_permission_profile(
...
fn network_access_from_policy(network_policy: NetworkSandboxPolicy) -> &'static str {
...
fn sandbox_text(mode: SandboxMode, network_access: &str) -> &'static str {
...
fn approval_text(
    ...
    let text = match approval_policy {
        ...
        AskForApproval::OnRequest => {
            ...
            with_request_permissions_tool(rule)
        }
        AskForApproval::Granular(granular_config) => {
            let mut sections = vec![
                "# Approval Requests\n\nApproval policy is `granular`. Categories set to `false` are automatically rejected instead of prompting the user.".to_string(),
            ];

            let categories = [
                Some((granular_config.allows_sandbox_approval(), "`sandbox_approval`")),
                ...
                Some((granular_config.allows_mcp_elicitations(), "`mcp_elicitations`")),
            ];
            ...
            if exec_permission_approvals_enabled && granular_config.allows_sandbox_approval() {
                sections.push(APPROVAL_POLICY_ON_REQUEST_RULE_REQUEST_PERMISSION.to_string());
            }
            if request_permissions_tool_enabled && granular_config.allows_request_permissions() {
