...
pub(crate) async fn run_inline_remote_auto_compact_task(
    ...
    if let Err((err, compaction_item)) = run_remote_compact_task_inner(
        &sess,
        &turn_context,
        Some(client_session),
    ...
    .await
    {
        info!(
            ...
            compact_error = %err,
            "remote compaction v2 failed; falling back to legacy remote compaction"
        );
        return crate::compact_remote::run_inline_remote_auto_compact_task_with_started_item(
            sess,
            turn_context,
            initial_context_injection,
            compaction_item,
...
pub(crate) async fn run_remote_compact_task(
    ...
    .await
    {
        ...
        return crate::compact_remote::run_remote_compact_task_after_turn_started_with_item(
            sess,
            turn_context,
            compaction_item,
        )
...
async fn run_remote_compact_task_inner(
    ...
    reason: CompactionReason,
    phase: CompactionPhase,
) -> Result<(), (CodexErr, TurnItem)> {
    let compaction_metadata = CompactionTurnMetadata::new(trigger, reason, phase);
...
async fn run_remote_compact_task_inner_impl(
    ...
    .await
    .map_err(|err| (err, compaction_item.clone()))?;
    ...
    let (compaction_output, response_id) = run_remote_compaction_request_v2(
        sess,
        turn_context,
        client_session,
        &prompt,
...
mod tests {
    ...
    fn build_v2_compacted_history_filters_to_installed_retention_shape() {
        ...
        assert_eq!(history, vec![message("user", "user", None), output]);
    }

    #[test]
    fn build_v2_compacted_history_discards_messages_before_truncating() {
        let old = message("user", "old", None);
        let new = message("user", "new", None);
        ...
        let input = vec![
            old.clone(),
            message("developer", &huge_developer_message, None),
    ...
    fn retained_history_truncation_keeps_newest_messages_first() {
        let middle = message("user", "middle1234", None);
        let new = message("user", "new", None);
        let retained = vec![message("user", "old-old", None), middle, new.clone()];

        let truncated = truncate_retained_messages_for_remote_compaction(retained, 3);

        assert_eq!(
            truncated,
            vec![message("user", "midd…1 tokens truncated…1234", None), new]
        );
    }
    ...
    fn retained_history_truncation_preserves_images_and_truncates_later_text_parts() {
        ...
        let truncated = truncate_retained_messages_for_remote_compaction(vec![item], 3);
    ...
    fn retained_history_truncation_charges_image_only_messages() {
        ...
        let newest = message("user", "new", None);
        let retained = vec![
        ...
        let truncated = truncate_retained_messages_for_remote_compaction(retained, 2);
    ...
    fn retained_history_truncation_drops_image_only_messages_after_budget_is_spent() {
        ...
        let truncated = truncate_retained_messages_for_remote_compaction(retained, 1);
