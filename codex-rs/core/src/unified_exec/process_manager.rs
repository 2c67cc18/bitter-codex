...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn exec_command(
        ...
        let process = self
            .open_session_unrestricted(&request, cwd.clone(), context)
            .await;
    ...
    pub(super) async fn open_session_unrestricted(
        &self,
        request: &ExecCommandRequest,
        cwd: AbsolutePathBuf,
        context: &UnifiedExecContext,
        ...
        let mut spawn_lifecycle: SpawnLifecycleHandle = Box::new(NoopSpawnLifecycle);
        let inherited_fds = spawn_lifecycle.inherited_fds();
        let spawn_result = if request.tty {
